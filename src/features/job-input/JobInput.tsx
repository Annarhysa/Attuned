      ...
      <CardContent className="pt-6">
        <Tabs defaultValue="paste">
          ...
          <TabsContent value="paste" className="mt-4 space-y-3">
            <Textarea rows={14} placeholder="Paste the job description here..." value={jd} onChange={(e) => setJd(e.target.value)} />
            <Button onClick={() => submitPasteOrKeywords('paste', jd)} disabled={loading}>{loading ? 'Analyzing...' : 'Analyze Job'}</Button>
          </TabsContent>
          ...
          <TabsContent value="keywords" className="mt-4 space-y-3">
            ...
            <Button onClick={() => submitPasteOrKeywords('keywords', keywords)} disabled={loading}>{loading ? 'Analyzing...' : 'Analyze Job'}</Button>
          </TabsContent>

          <TabsContent value="upload" className="mt-4">
            <label className="flex cursor-pointer flex-col items-center gap-2 rounded-lg border-2 border-dashed border-border p-10 text-center hover:border-primary">
              <Upload className="h-8 w-8 text-muted-foreground" />
              <span className="text-sm font-medium">Click to upload PDF, DOCX, or TXT</span>
              <input type="file" accept=".pdf,.docx,.txt" className="hidden" onChange={(e) => e.target.files?.[0] && submitFile(e.target.files[0])} />
            </label>
          </TabsContent>
        </Tabs>
